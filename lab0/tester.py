# Python 3 compatible version of tester.py
import traceback
import sys
import os
import tarfile
...
# Placeholder server credentials/URL — set these before calling test_online()
server_url = None
username = None
password = None
...
def run_test(test, lab):
    ...
    if type == 'VALUE':
        ...
        raise Exception("Test Error: Unknown TYPE '%s'.  Please make sure you have downloaded the latest version of the tester script.  If you continue to see this error, contact a TA." % type)
...
def get_tarball_data(target_dir, filename):
    data = BytesIO()
    tar = tarfile.open(fileobj=data, mode="w:bz2")

    print("Preparing the lab directory for transmission...")
            
    # Add the directory using its basename so the tarball is not full-path anchored
...
def test_online(verbosity=1):
    ...
    try:
        server = xmlrpc_client.ServerProxy(server_url, allow_none=True)
        ...
        if len(answer) == 0 or answer[0] in "Yy":
            server = xmlrpc_client.ServerProxy(server_url.replace("https", "http"))
            tests = server.get_tests(username, password, lab.__name__)
        else:
    ...
    server.submit_code(username, password, lab.__name__, xmlrpc_client.Binary(tarball_data))
